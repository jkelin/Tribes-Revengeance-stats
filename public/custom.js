function requires(scriptNames, cb) {
    var loaded = scriptNames.filter(function(n){ return window._scripts[n] }).reduce(function(a){ return a + 1}, 0);
...
requires(['zepto', 'socketio'], function () {
    ...
    if (chatElem) {
        ...
        var chatData = JSON.parse(chatElem.innerHTML) || [];
        ...
        socket.on('chat-message', function (data) {
            ...
            var html = '<div class="chat-item">' +
                ...
                '<span class="chat-message">' + data.messageFriendly + '</span>' +
                '</div>';
